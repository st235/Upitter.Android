package com.github.sasd97.upitter.constants;

/**
 * Created by Alexander Dadukin on 06.06.2016.
 */
public interface MethodConstants {

    String BASE_API_URL = "http://192.168.1.105:9000/";
    String FILE_SERVER_API_URL = "http://127.0.0.1:3000/";

    String BASE_SUB_API_URL = "http://192.168.1.4:9000/";
    String FILE_SUB_SERVER_API_URL = "http://192.168.1.4:3000/";

    String BASE_SUB_ASTRAL_API_URL = "http://192.168.1.230:9000/";
    String FILE_SUB_ASTRAL_SERVER_API_URL = "http://192.168.1.230:3000/";
}
